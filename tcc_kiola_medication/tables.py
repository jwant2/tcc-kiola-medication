...
class MedicationAdherenceOverview(object):
    ...
    def render(self):
        ...
        headers = ['']
        for i in range(0, days): headers.append((self.start+timedelta(days=i+1)).strftime("%d/%m"))
        data = []
        for schedule in schedules:
            row_data = []
            row_data.append(dict(
                value=("%s (%s)" % (schedule.compound_name, schedule.get_displayable())),
                cell_class="col-md-4"
            ))
            for i in range(0, days): row_data.append(dict(color_class="", value=""))
            schedule_data = self._get_obs_data(self.request, schedule.pk, self.start, self.stop)
    ...
    def _get_obs_data(self, request, schedule_id, start=None, stop=None, count_only=False, **kwargs):
        subject_uid = self.request.subject_uid
        observations = (
