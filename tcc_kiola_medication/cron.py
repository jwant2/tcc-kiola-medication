...
from kiola.kiola_export import const as export_const
from kiola.kiola_export import models as export_models
...
class ScheduleTakingReminderJob(CronJobBase):
    ...
    def _should_send_reminder(self, subject, compound_name, time, now):
        ...
        # skip checking
        if time_diff < const.MEDICATION_REMINDER_TIME_MINUTES or time_diff > 60:
            return False
    ...
    def _get_taking_time(self, subject, taking):
        if taking.timepoint.name == "custom":
            ...
            user_pref = models.UserPreferenceConfig.objects.get_value(
                const.USER_PREFERENCE_KEY__MEDICATION_TIMES, subject.login
            )
            ...
            time_value = user_pref[
                f"{const.USER_PREFERENCE_CONFIG_PREFIX}{taking.timepoint.name}"
            ]
    ...
    def do(self):
        ...
        with reversion.create_revision():
            ...
            schedule_takings = models.ScheduledTaking.objects.filter(
                Q(end_date__gte=now, start_date__lte=now)
            ...
            for taking in takings:
                try:
                    prescription = models.TCCPrescription.objects.prefetch_related(
                        "subject", "compound"
                    ).get(pk=taking.prescr_id)
                    subject = prescription.subject
                    time = self._get_taking_time(subject, taking)
                    should_send = self._should_send_reminder(
                        subject, prescription.compound.name, time, now
                    )
                    if not should_send:
                        continue
                    try:
                        utils.send_medication_reminder_notification(
                            subject, taking, time, prescription.compound.name
                        )
                        ...
                        BackendNotification.objects.create_backend_notification(
                            get_system_user(), err_msg, traceback.format_exc()
                        )
