...
class CompoundImportHistoryForm(ModelForm):
    ...
    data_source_description = forms.CharField(label=_("Data source description"), required=False)
    ...
    def clean(self):
        ...
        exist = med_models.CompoundSource.objects.filter(name=const.COMPOUND_SOURCE_NAME__TCC, version=cleaned_data['data_source_version']).count() > 0
...
class TCCPrescriptionDataLoader(kiola_forms.InitialDataLoader):
    ...
    def load(self, request, fid=None, **kwargs):
        ...
        if fid is not None:
            ...
            end = prescription.prescriptionevent_set.filter(etype=med_models.PrescriptionEventType.objects.get(name=med_const.EVENT_TYPE__END))
            if len(end) > 0:
            ...
            try:
                data["compound_source"] = med_models.CompoundSource.objects.filter(default=True).order_by("-pk")[0].pk
            except IndexError as details:
                raise AttributeError("No default compound source found - has the med database been properly set up ?")
            data["taking_unit"] = med_models.TakingUnit.objects.get(name=med_const.TAKING_UNIT__UNIT).pk
...
class TCCPrescriptionForm(senses_forms.KiolaSubjectForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["ev__prescription_startdate"] = forms.DateField(
            label=_(u"Date of prescription"),
            required=True,
            initial=now().date(),
            **date_options
        )
        ...
        self.fields["status"] = forms.IntegerField(widget=forms.HiddenInput(), required=False)
        ...
        self.fields["taking_reason"] = forms.CharField(label=_("Reason of prescription"),
                                                       required=False,
                                                       max_length=100)
        self.fields["taking_hint"] = forms.CharField(label=_("Hint for taking"),
                                                     required=False,
        ...
        choices = [
          ...
          (const.MEDICATION_TYPE_VALUE__REGULAR, const.MEDICATION_TYPE_VALUE__REGULAR)
        ]
        self.fields["med_type"] = forms.ChoiceField(choices=choices,
                                                      widget=themes_widgets.RadioSelect(renderer=cares_widgets.HorizontalRadioRenderer,
                                                      attrs={"wrapper-class": "radio-option-space2x"},),
                                                      required=True,
                                                      label="Medication type(SCH/PRN) *")

        self.fields["taking"] = forms.CharField(widget=forms.HiddenInput(), required=False)


        date_options = signal_registry.send_to_category("request_datetime_field_options", self, {"fieldtype": "date"})[0][1][0][1]
        self.fields["ev__prescription_enddate"] = forms.DateField(
            label=_(u"Stop taking"),
            required=False,
            **date_options
        )
        ...
        if getattr(self, "_kiola_option__disabled", False):
            ...
            # the buttons
            if self.fid is None:
                self.buttons.append(kiola_forms.SubmitButton(ugettext_noop("Save"), self.savenew))
            else:
                self.buttons.append(kiola_forms.SubmitButton(ugettext_noop("Save"), self.update_or_create))

        self.buttons.insert(0, kiola_forms.BackToListButton(ugettext_noop("Back"), None, {'listname': 'med:prescription_index',
    ...
    def savenew(self, request, proxy=False):
        ...
        subject = senses_models.Subject.objects.get(uuid=request.subject_uid)
        compound = med_models.Compound.objects.get(uid=cd["compound_id"], source__pk=cd['compound_source'])
        ...
        if not proxy:
            messages.success(request, _("Prescription added: {compound}").format(compound=compound.name))
...
class ScheduleTakingForm(ModelForm):
    ...
    active = forms.BooleanField(widget=forms.HiddenInput(), initial=True, required=False)
    class Meta:
        js = ('autocomplete.js', 'text_widget.js')
        css = {
            'all': ['fa-time.css',
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["timepoint"].queryset = med_models.TakingTimepoint.objects.filter(name__in=["morning", "noon", "afternoon", "night", "custom"]).order_by('pk')
        self.fields["timepoint"].required = False
        time_options = signal_registry.send_to_category("request_datetime_field_options", self, {"fieldtype": "time"})[0][1][0][1]
        self.fields["taking_time"] = forms.TimeField(
            label=_(u"Custom time of taking medication"),
            required=False,
            **organizer_utils.time_options()
        )
        self.fields['taking_time'].widget.attrs.update({
            'autocomplete': 'off'
        })
        date_options = signal_registry.send_to_category("request_datetime_field_options", self, {"fieldtype": "date"})[0][1][0][1]
        self.fields["start_date"] = forms.DateField(
            label=_(u"Start date of taking medication *"),
            required=True,
            initial=now().date(),
            **date_options
        )        
