...
def set_default_user_pref_med_time_values(user):
    models.UserPreferenceConfig.objects.set_value(
        const.USER_PREFERENCE_KEY__MEDICATION_TIMES,
        const.MEDICATION_TIMES_DEFAULT_VALUES,
        user,
    )
...
def tcc_render_sis_compound_info(context, compound_id, compound_source=None):
    try:
        ...
        if compound.source.pk != current_compound_source.pk:
            ...
            try:
                new_compound = med_models.Compound.objects.get(
                    uid=compound_id, source=current_compound_source
                )

                context["compound_newer"] = {}
                context["compound_newer"][
                    "source_version"
                ] = current_compound_source.version
                context["compound_newer"]["title"] = new_compound.name
                context["compound_newer"]["unique_id"] = compound_id
                context["compound_newer"]["main_indications"] = ", ".join(
                    new_compound.indications.all().values_list("name", flat=True)
                )
                context["compound_newer"]["active_components"] = ", ".join(
                    new_compound.active_components.all()
                ...
                if len(drugs) > 0:
                    ...
                    context["compound_newer"]["main_indications"] = list(
                        new_compound["main_indications"].values()
                    )[0]
                    context["compound_newer"]["active_components"] = ", ".join(
                        sorted(new_compound["active_components"].values())
                    )
                    context["compound_newer"]["dosage_form"] = list(
                        new_compound["dosage_form"].values()
                    )[0]
                else:
        ...
        context["main_indications"] = ", ".join(
            compound.indications.all().values_list("name", flat=True)
        )
        context["active_components"] = ", ".join(
            compound.active_components.all()
            .values_list("name", flat=True)
            .order_by("name")
        )
        ...
        context["active_components"] = ", ".join(
            sorted(compound["active_components"].values())
        )
...
med_tags.register.inclusion_tag("tcc_sis_search_result.html", takes_context=True)(
    tcc_render_sis_compound_info
)
...
class TCCAdapter(med_utils.CompoundAdapterBase):
    def get_or_create(self, compound_id):
        ...
        compounds = self.manager.filter(uid=compound_id, source=self.source)
        ...
        if len(compounds) == 0:
            ...
            for unique_id, comp in product["active_components"].items():
                if comp:
                    (
                        active_component,
                        created,
                    ) = med_models.ActiveComponent.objects.get_or_create(name=comp)
            ...
            raise djexceptions.MultipleObjectsReturned(
                "Found more than one compound, matching id and source"
            )
        else:
...
def filter_schedule_for_given_date(given_date, qs):
    ...
    for taking in qs:
        ...
        if (
            taking.frequency.name == const.TAKING_FREQUENCY_VALUE__ONCE
            ...
            taking.frequency.name == const.TAKING_FREQUENCY_VALUE__WEEKLY
            and current_date.weekday() == date.weekday()
        ):
            filtered_ids.append(taking.id)
        elif (
            taking.frequency.name == const.TAKING_FREQUENCY_VALUE__FORTNIGHTLY
            and (current_date - date).days % 14 == 0
            and current_date.weekday() == date.weekday()
        ):
            filtered_ids.append(taking.id)
        elif (
            taking.frequency.name == const.TAKING_FREQUENCY_VALUE__MONTHLY
            and (current_date - date).days % 30 == 0
        ):
            filtered_ids.append(taking.id)
    return models.ScheduledTaking.objects.filter(id__in=filtered_ids).annotate(
        prescr_id=F("takingschema__prescriptionschema__prescription")
    )


def send_medication_reminder_notification(
    subject, taking, time, compound_name, title=const.DEFAULT_MEDICATION_REMINDER_TITLE
):
