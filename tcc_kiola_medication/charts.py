...
class TCCMedicationComplianceChart(cares_charts.ChartBase):
    ...
    def get(self, subject, which, period, ajax=False, data_filters={}):
        ...
        plot = super(TCCMedicationComplianceChart, self).get(
            subject, which, period, ajax, data_filters
        )
        chart_options = copy.deepcopy(self.chart_options)

        flt = {
            "parent__started__range": period,
        }
        ...
        for counter, series in enumerate(self.series):
            observations = (
                ...
                .annotate(
                    schedule=Subquery(
                        senses.DateTimeSimpleObservation.accepted.filter(
                ...
                .annotate(
                    prescr_id=Subquery(
                        senses.TextObservation.accepted.filter(
                            parent__pk=OuterRef("parent__pk"),
            ...
            for observation in observations:
                try:
                    compound = models.TCCPrescription.objects.get(
                        pk=observation.prescr_id
                    ).compound.name
                except models.TCCPrescription.DoesNotExist:
                    compound = None  # this means observation data is invalid
                pointdata = {
                    "x": observation.parent.started.replace(hour=0, minute=0, second=0),
...
cares_modules.AllChartsModule.setoption(
    const.CHART_CATEGORY__TCC_MEDICATION, "h=200&ldrag=1"
)
