...
urlpatterns = [
    ...
    url(
        r"compound/(?P<default>default)/$",
        views.CompoundAPIView.as_view(),
        name="compound-default",
    ),
    url(r"compound/$", views.CompoundAPIView.as_view(), name="compound"),
    url(
        r"compound/(?P<id>[a-zA-Z0-9\-_]*)/$",
        views.CompoundAPIView.as_view(),
    ...
    url(
        r"medication/(?P<id>\d+)/$",
        views.PrescriptionAPIView.as_view(),
        name="single-medication",
    ),
    url(
        r"medication/(?P<id>\d+)/history/$",
        views.PrescriptionHistoryAPIView.as_view(),
        name="medication-history",
    ),
    url(
        r"user_preference/solar_option/$",
        views.UserPreferenceConfigAPIView.as_view(),
        name="user_preference_config",
    ),
    url(r"schedule/$", views.TakingSchemaAPIView.as_view(), name="taking"),
    url(
        r"schedule/(?P<id>\d+)/$",
        views.TakingSchemaAPIView.as_view(),
        name="single-taking",
    ),
    url(
        r"reaction/$",
        views.MedicationAdverseReactionAPIView.as_view(),
        name="med_adverse_reaction",
    ),
    url(
        r"reaction/(?P<id>[a-zA-Z0-9\-_]*)/$",
        views.MedicationAdverseReactionAPIView.as_view(),
        name="single-med_adverse_reaction",
    ),
]
