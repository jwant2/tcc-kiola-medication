...
from kiola.utils import exceptions, service_providers
from kiola.utils import views as kiola_views
from kiola.utils.authentication import requires_api_login
from kiola.utils.commons import get_system_user, http_client_codes
        ...
        if id:
            ...
            if qs.count() > 0:
                ...
                raise exceptions.BadRequest(
                    "Given compound id '%s' does not exists " % id
                )
            ...
            qs = qs.filter(
                Q(source__default=True)
                | Q(source__version=const.COMPOUND_SOURCE_VERSION__PATIENT)
            )
            ...
            if compound_name:
                ...
                #     return Response(msg, status=status.HTTP_200_OK)
                qs = template.filter(
                    active_components__name__icontains=active_component
                )
            ...
            rest_list = qs.exclude(
                id__in=default_list.values_list("id", flat=True)
            ).order_by("name")
    ...
    def post(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        try:
            ...
            if hasattr(err, "message"):
                msg = err.message
            raise exceptions.BadRequest(
                "Invalid data '%s'. Missing some of the required fields. Error: '%s' "
                % (request.data, msg)
            )
        ...
        if exist > 0:
            raise exceptions.BadRequest(
                "Given compound name '%s' already exists " % compound_name
            )
...
class MedObservationProfileAPIView(APIView):
    ...
    def get(self, request, subject_uid=None, uid=None, *args, **kwargs):
        ...
        active_ppr = med_models.PrescriptionProfileRelation.objects.filter(
            active=True, root_profile__subject=subject
        ).first()
        if active_ppr:
            root_profile = active_ppr.root_profile
            data = [{"id": root_profile.name}]
            ...
            if len(children) > 0:
                ...
                for child in children:
                    ...
                    data[0]["children"].append({"id": child.name})
        else:
            data = []
...
class PrescriptionAPIView(APIView, PaginationHandlerMixin):
    ...
    def get(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        if prescr_id:
            try:
                prescr = (
                    models.TCCPrescription.objects.select_related("compound", "status")
                    .filter(
                        pk=prescr_id,
                        subject=subject,
                    ...
                    .order_by("compound__name", "status")[0]
                )

            except:
                raise exceptions.BadRequest(
                    "Prescription with id '%s' does not exist or is inactive"
                    % prescr_id
                )
            ...
            if active:
                if active not in ["True", "False", "true", "false"]:
                    raise exceptions.BadRequest(
                        "Invalid data '%s' for active. Should be 'true' or 'false'."
                        % active
                    )
                active = active.lower()
            qs = (
                ...
                .order_by("compound__name", "status")
            )
            if active == "true":
    ...
    def delete(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        try:
            ...
            raise exceptions.Forbidden("Unknown subject")
        try:
            prescription = models.TCCPrescription.objects.get(
                subject=subject,
                pk=prescr_id,
                status__name=med_const.PRESCRIPTION_STATUS__ACTIVE,
            )
            ...
            raise exceptions.NotFound(
                "Prescription with id '%s' does not exist or is inactive" % prescr_id
            )

        p_status = med_models.PrescriptionStatus.objects.get(
    ...
    def post(self, request, subject_uid=None, id=None, *args, **kwargs):
        if id is not None:
            raise exceptions.BadRequest(
                "Creating new resource with given id is not supported."
            )
        try:
            if subject_uid is None:
                subject = senses.Subject.objects.get(login=request.user)
    ...
    def _create_or_update(self, request, prescr_id, subject):
        ...
        def process_request_data(request_data):
            data = {
                ...
                "medicationAdverseReactions": request_data.get(
        ...
        processed_data = process_request_data(
            request.data
        )  ## filter unnessary data fields
        ...
        end_date = None
        if p_end:
            end_date = datetime.strptime(p_end.split("T")[0], "%Y-%m-%d").astimezone(
                timezone.utc
            )

        if medication_type and medication_type not in const.MEDICATION_TYPE_VALUES:
            raise exceptions.BadRequest(
                "Invalid data '%s' for medicationType" % medication_type
            )
        ...
        if prescr_id:
            try:
                prescr = models.TCCPrescription.objects.get(
                    pk=prescr_id, status__name=med_const.PRESCRIPTION_STATUS__ACTIVE
                )
            ...
            if prescr.compound.uid != compound_obj["id"]:
                raise exceptions.BadRequest(
                    "Compound with id '%s' does not match the compound of the given prescription with id '%s'"
                    % (compound_obj["id"], prescr_id)
                )
            ...
            start = prescr.prescriptionevent_set.filter(
                etype=med_models.PrescriptionEventType.objects.get(
            ...
            start.save()
            end = prescr.prescriptionevent_set.filter(
                etype=med_models.PrescriptionEventType.objects.get(
            ...
            if active is not None and active is False:
                prescr_stauts_inactive = med_models.PrescriptionStatus.objects.get(
                    name=med_const.PRESCRIPTION_STATUS__INACTIVE
                )
            ...
            with reversionrevisions.create_revision():
                reversionrevisions.set_user(get_system_user())
                try:
                    adapter = med_models.Compound.objects.get_adapter(
                        med_models.CompoundSource.objects.get(default=True).pk
                    )
                    ...
                    try:
                        ...
                        compound = med_models.Compound.objects.get(
                            uid=compound_obj["id"], source=source
                        )
                    except:
                        raise exceptions.BadRequest(
                            "Compound with id '%s' does not exist" % compound_obj["id"]
                        )
                ...
                med_type = models.MedicationType.objects.get(name=medication_type)
                prescr, replaced = models.TCCPrescription.objects.prescribe(
                    subject=subject,
        ...
        record = models.MedicationRelatedHistoryData.objects.add_data_change_record(
            prescr, processed_data
        )

        return prescr


def update_or_create_med_adverse_reaction(
    request, reactions_str: str, compound: med_models.Compound
):
    reactions = reactions_str.split(",")
    reaction_type = models.AdverseReactionType.objects.get(
        name=const.ADVERSE_REACTION_TYPE__UNKNOWN
    )
    for item in reactions:
        exist = models.MedicationAdverseReaction.objects.filter(
            compound=compound, reactions=reactions, editor=request.user
        ).count()
        if exist == 0 and item != "":
            (
                reaction_item,
                created,
            ) = models.MedicationAdverseReaction.objects.get_or_create(
                compound=compound,
...
def update_prescription_displayable_taking(prescription):
    takings = models.ScheduledTaking.objects.filter(
        takings_set__id=prescription.id, active=True
    )
    taking_strings = []
...
class PrescriptionHistoryAPIView(APIView, PaginationHandlerMixin):
    ...
    def get(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        try:
            prescr = models.TCCPrescription.objects.select_related("compound").get(
                pk=id, status__name=med_const.PRESCRIPTION_STATUS__ACTIVE
            )
        except Exception as err:
            print(err)
            raise exceptions.BadRequest(
                "Prescription with id '%s' does not exist or is inactive" % id
            )
        ...
        for i in range(len(qs) - 1):
            ...
            if value:
                ...
                for item in value:
                    ...
                    temp.append(data)
                results.append({"time": force_text(qs[i + 1].created), "changes": temp})
        if page is not None:
...
class MedicationAdverseReactionAPIView(APIView, PaginationHandlerMixin):
    ...
    def get(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        if reaction_id:
            try:
                reaction_item = models.MedicationAdverseReaction.objects.get(
                    uid=reaction_id
                )
            except Exception as err:
                print(err)
                raise exceptions.BadRequest(
                    "MedicationAdverseReaction with id '%s' does not exist"
                    % reaction_id
                )
    ...
    def delete(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        try:
            reaction_item = models.MedicationAdverseReaction.objects.get(
                uid=reaction_id, editor=subject.login, active=True
            )
        except:
            raise exceptions.BadRequest(
                "MedicationAdverseReaction with id '%s' does not exist or is inactive"
                % reaction_id
            )
        with reversionrevisions.create_revision():
            reversionrevisions.set_user(get_system_user())
    ...
    def put(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        reaction_item = self._create_or_update(
            request, reaction_id, compound_id, reaction_type_name, reactions
        )
        serializer = self.serializer_class(reaction_item)
    ...
    def _create_or_update(
        self, request, reaction_id, compound_id, reaction_type_name, reactions
    ):
        ...
        try:
            ...
            raise exceptions.BadRequest(
                "AdverseReactionType with name '%s' does not exist" % reaction_type_name
            )
        ...
        if reaction_id:
            try:
                reaction_item = models.MedicationAdverseReaction.objects.get(
                    uid=reaction_id, editor=request.user, active=True
                )
            except Exception:
                raise exceptions.BadRequest(
                    "MedicationAdverseReaction with id '%s' does not exist or is inactive"
            ...
            if reaction_item.compound.uid != compound_id:
                raise exceptions.BadRequest(
                    "Compound id '%s' does not match the gvien reaction (%s)"
            ...
            try:
                ...
                try:
                    ...
                    compound = med_models.Compound.objects.get(
                        uid=compound_id, source=source
                    )
                except:
                    raise exceptions.BadRequest(
                        "Compound with id '%s' does not exist" % compound_id
                    )
...
class TakingSchemaAPIView(APIView, PaginationHandlerMixin):
    ...
    def get(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        if taking_id:
            try:
                taking_item = models.ScheduledTaking.objects.annotate(
                    prescr_id=F("takings_set__id")
                ).get(pk=taking_id)
            except Exception as err:
                print(err)
                raise exceptions.BadRequest(
                    "ScheduledTaking with id '%s' does not exist" % taking_id
                )
            ...
            taking_qs = models.ScheduledTaking.objects.filter(
            ...
            if start_date:
                try:
                    ...
                    raise exceptions.BadRequest(
                        "Invalid datetime format '%s' for startDate. " % start_date
                    )
            ...
            if end_date:
                try:
                    ...
                    raise exceptions.BadRequest(
                        "Invalid datetime format '%s' for endDate. " % end_date
                    )
            ...
            if given_date:
                try:
                    ...
                    raise exceptions.BadRequest(
                        "Invalid datetime format '%s' for endDate. " % given_date
                    )
                taking_qs = taking_qs.filter(
                    Q(end_date__gte=given, start_date__lte=given)
                    | Q(start_date__lte=given, end_date=None)
                )
    ...
    def delete(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        taking = None
        try:
            taking = models.ScheduledTaking.objects.annotate(
                prescr_id=F("takings_set__id")
            ).get(pk=taking_id, active=True)
        except:
            raise exceptions.BadRequest(
                "Taking with id '%s' does not exist or is inactive" % taking_id
            )
    ...
    def post(self, request, subject_uid=None, id=None, *args, **kwargs):
        ...
        try:
            ...
            raise exceptions.BadRequest(
                "Invalid data '%s'. Missing some of the required fields " % data
            )

        taking = process_taking_request(
            request,
            data,
            None,
            prescr_id,
        ...
        serializer = self.serializer_class(
            models.ScheduledTaking.objects.annotate(prescr_id=F("takings_set__id")).get(
                pk=taking.pk
            )
        )
...
def process_taking_request(
    ...
    if taking_id:
        try:
            ...
            raise exceptions.BadRequest(
                "Taking with id '%s' does not exist " % taking_id
            )
    ...
    if schedule_type == "custom":
        ...
        user_pref = models.UserPreferenceConfig.objects.get_value(
            const.USER_PREFERENCE_KEY__MEDICATION_TIMES, request.user
        )
    ...
    if taking:
        if taking.pk not in takings:
            raise exceptions.BadRequest(
                "Taking with id '%s' does not match the gvien prescription (%s)"
                % (taking_id, prescr_id)
            )
        ...
        taking.end_date = (
            datetime.strptime(end_date.split("T")[0], "%Y-%m-%d") if end_date else None
        )
...
class UserPreferenceConfigAPIView(APIView, PaginationHandlerMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        data = models.UserPreferenceConfig.objects.get_value(
            const.USER_PREFERENCE_KEY__MEDICATION_TIMES, request.user
        )
        if data is None:
            set_default_user_pref_med_time_values(request.user)
    ...
    def put(self, request, *args, **kwargs):
        ...
        try:
            validate(
                instance=request.data, schema=request_schema.UpdateUserPreferenceBody
            )
        except Exception as err:
            msg = err
            if hasattr(err, "message"):
        ...
        config_data = models.UserPreferenceConfig.objects.get_value(
            const.USER_PREFERENCE_KEY__MEDICATION_TIMES, request.user
        )
        ...
        for item in med_pref_data:
            ...
            if item["type"] not in dict_check_duplicates:
                ...
                raise exceptions.BadRequest(
                    "Duplicate data '%s' for type" % item["type"]
                )
            ...
            try:
                ...
                raise exceptions.BadRequest(
                    "actualTime data '%s' is invalid or not in 24 hr format"
                    % item["actualTime"]
                )
            config_item = config_data.get(
                f'{const.USER_PREFERENCE_CONFIG_PREFIX}{item["type"]}', None
            )
        ...
        with reversionrevisions.create_revision():
            reversionrevisions.set_user(get_system_user())
        result = models.UserPreferenceConfig.objects.set_value(
            const.USER_PREFERENCE_KEY__MEDICATION_TIMES, config_data, request.user
        )
        config_data = models.UserPreferenceConfig.objects.get_value(
            const.USER_PREFERENCE_KEY__MEDICATION_TIMES, request.user
        ).values()
        ...
        for item in processed_items:
            ...
            models.ScheduledTaking.objects.filter(
                Q(timepoint__name=item["type"])
            ).update(taking_time=time)
...
class TCCPrescriptionListView(kiola_views.KiolaSubjectListView):
    ...
    def get_queryset(self):
        ...
        qs = (
            models.TCCPrescription.objects.select_related("compound", "status")
            .filter(subject=subject, status__name=med_const.PRESCRIPTION_STATUS__ACTIVE)
            .prefetch_related(
                Prefetch(
                    "prescriptionevent_set",
                    queryset=med_models.PrescriptionEvent.objects.filter(
    ...
    def get_context_data(self, **kwargs):
        ...
        inactive_qs = (
            models.TCCPrescription.objects.select_related("compound", "status")
            .filter(
                subject=subject, status__name=med_const.PRESCRIPTION_STATUS__INACTIVE
            )
            .prefetch_related(
                Prefetch(
                    "prescriptionevent_set",
                    queryset=med_models.PrescriptionEvent.objects.filter(
                        etype__name__in=[
                            ...
                            med_const.EVENT_TYPE__REPLACED,
                        ]
                    )
                    .select_related("etype")
                    .order_by("timepoint"),
        ...
        for prescr in context["inactive_prescriptions"]:
            takings = models.ScheduledTaking.objects.filter(
                takings_set__id=prescr.id, active=True
            )
            taking_strings = []
            for taking in takings:
                taking_strings.append(taking.get_displayable())
        ...
        context["status"] = {
            "inactive": med_const.PRESCRIPTION_STATUS__INACTIVE,
            "hidden": med_const.PRESCRIPTION_STATUS__HIDDEN,
        }
        context["enddatekey"] = med_const.EVENT_TYPE__END
        context[
            "prescription_profiles_active"
        ] = settings.KIOLA_PRESCRIPTION_PROFILES_ENABLED
        pprelation = med_models.PrescriptionProfileRelation.objects.filter(
            active=True, root_profile__subject=subject
        )
        context["active_profile_relation"] = pprelation
        if len(pprelation) > 0:
            context["active_profile_ids"] = (
                pprelation[0].prescriptions.all().values_list("pk", flat=True)
            )
        return context
...
class TCCPrescriptionView(kiola_views.KiolaSubjectView):
    ...
    def get_form_kwargs(self):
        ...
        if kwargs["initial"].get("status", None) not in [
            med_models.PrescriptionStatus.objects.get(
    ...
    def get_context_data(self, **kwargs):
        ...
        capsule_translated = (
            _(med_const.TAKING_UNIT__CAPSULE).translate(language_code).lower()
        )
        search_list = [
            _(med_const.TAKING_UNIT__PILL).translate(language_code).lower(),
            _(med_const.TAKING_UNIT__CAPSULE).translate(language_code).lower(),
        ...
        pattern_list = pharmacy_models.ProductDosageForm.objects.filter(
            reduce(operator.or_, (Q(title__icontains=x) for x in search_list))
        ).values_list("title", flat=True)

        kwargs["taking_unit_mapping"] = {
            "pattern_list": list(pattern_list),
            "value": med_models.TakingUnit.objects.get(
                name=med_const.TAKING_UNIT__PILL
            ).pk,
        }
        kwargs["taking_unit_default_value"] = med_models.TakingUnit.objects.get(
            name=med_const.TAKING_UNIT__UNIT
        ).pk
        if "form" not in kwargs:
            kwargs["form"] = self.get_form()

        if kwargs["form"].initial.get("status", None) not in [
            med_models.PrescriptionStatus.objects.get(
                name=med_const.PRESCRIPTION_STATUS__ACTIVE
            ).pk,
            None,
        ...
        if self.fid:
            prescr = models.TCCPrescription.objects.select_related("unit").get(
                id=self.fid
            )
            kwargs["taking_form"] = forms.ScheduleTakingForm(
                initial={
            ...
            active_takings = models.ScheduledTaking.objects.filter(
                takings_set__id=self.fid, active=True
            )
            kwargs["active_takings"] = active_takings
            inactive_takings = models.ScheduledTaking.objects.filter(
                takings_set__id=self.fid, active=False
            )
            kwargs["inactive_takings"] = inactive_takings
            reactions = models.MedicationAdverseReaction.objects.filter(
                compound=prescription.compound, editor=subject.login
            )
...
class TakingSchemaResource(resource.Resource):
    ...
    def post(self, request, sid=None, id=None, fid=None, **kwargs):
        ...
        try:
            ...
            schedule_time = (
                data["taking_time"].strftime("%H:%M")
                if data["taking_time"] and data["timepoint"].name == "custom"
                else data["timepoint"].name
            )
            ...
            end_date = (
                data.get("end_date").strftime("%Y-%m-%d")
...
class TCCAutocompleteResource(resource.Resource):
    ...
    def get(self, request, **kwargs):
        ...
        try:
            ...
            out = '<span style="color:red">%s</span>' % _(
        ...
        try:
            ...
            # FIXME : log in backend
            msg = _(
                "At the moment we are updating the database. Please try again later."
            )
            out = '<span style="color:red">%s</span>' % msg
            return http.HttpResponse(out, content_type="text/html")
        if len(drugs) > self.max_count:
            out = (
                "<span>%s</span>"
                % _(
                    "More than %(max)s results found (%(amount)s). Please refine your search.."
                ).translate(get_language())
                % {"max": self.max_count, "amount": len(drugs)}
            )
            return http.HttpResponse(out, content_type="text/html")
        ...
        results = []
        for drug in drugs:
            prn = models.CompoundExtraInformation.objects.filter(
                compound__uid=drug["unique_id"],
            ...
            context = {
                ...
                "active_components": ", ".join(
                    sorted(drug["active_components"].values())
                ),
                "prn": prn.value if prn else None,
                "count": drug["count"],
            }
            results.append(t.render(context))
        html = '<span class="total_count">%s %s %s</span>\n' % (
            _("Found").translate(get_language()),
            len(drugs),
            _("results").translate(get_language()),
        )
        html += " (%s)\n" % (
